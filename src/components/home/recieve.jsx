...
const Receive = () => {
  return (
    <div id="benefits" className="py-10 md:pt-20 pt-0 md:py-20 px-5  ">
      <div className="max-w-[1270px] rainbow-border z-30 relative p-[1px] mx-auto  rounded-[24px]">
          ...
          <div className="grid px-[24px] md:px-[66px] place-items-center mt-[50px] grid-cols-1 md:grid-cols-2 gap-10">
            {Receive_Section_data.map((item, i) => (
              <div
                ...
                key={i}
              >
                <div className="md:block hidden">{item.lgIcon}</div>
                <div className="block md:hidden">{item.smIcon}</div>
                <div className="space-y-[6px] md:space-y-2">
                  <div className="flex md:justify-start justify-center gap-x-[6px] md:gap-x-[11px] items-center">
                    <h3 className="bg-clip-text text-[18px] md:text-[24px] font-extrabold text-transparent bg-gradient-to-r from-[#FFA1D7] to-[#F71B9A]">
                      {item.title}
                    </h3>
